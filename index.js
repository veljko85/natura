...
var createScene = function () {
  ...
  function changePosAndScaleFence(valueToCount, activeFence) {
    ...
    if (fencesArr[activeFence].type == "natura90180")
  ...
  // MAIN POST MESH //////////////////////////////////////////////////////////////////////////
  BABYLON.SceneLoader.ImportMeshAsync("", "mesh/", "mainPostNatura2.glb").then(
    (result) => {
  ...
  var createRightFence = (posX, posZ, rotY, type) =>
    BABYLON.SceneLoader.ImportMeshAsync("", "mesh/", "naturaCombo15.glb").then(
      (result) => {
        ...
        function toActiveFence() {
          ...
          if (fencesArr[activeFence].type == "natura180180")
          ...
          if (fencesArr[activeFence].color == "braun")
        ...
        // fakeFence.scaling.x = 3
        fakeFence.position = new BABYLON.Vector3(
          getAbsPosX(result.meshes[1]) - 0.015,
          0.9,
        ...
        let natura180180 = result.meshes[1];
        natura180180Arr.push(natura180180);
        natura180180.isVisible = false;
        let natura90180 = result.meshes[2];
        natura90180Arr.push(natura90180);
        natura90180.isVisible = false;

        let allFencesThis = new Array(natura180180, natura90180);

        allFences.push(allFencesThis);

        if (type == "natura180180") natura180180.isVisible = true;

        if (type == "natura90180") {
          natura90180.isVisible = true;
          setSmallerFenceSize(
            rightPost,
            rightPostCap,
            newFenceForwardSigns,
  ...
  function handleTabActivnes() {
    if (!document.hidden) {
      createRightFence(0.99, 0, 0, "natura180180");
  ...
  function setPhostenType(a, b) {
    ...
    if (
      fencesArr[0].type == "sina18090" ||
      fencesArr[0].type == "sinaKosaLeva"
    ) {
  ...
  function changeFenceFunction(c, d, h) {
    ...
    if (d == "natura180180") {
      natura180180Arr[h].isVisible = true;
    }
    if (d == "natura90180") {
      natura90180Arr[h].isVisible = true;
    }
  ...
  changeFence[0].addEventListener("click", () => {
    changeFenceFunction(180, "natura180180", activeFence);
  });
  changeFence[1].addEventListener("click", () => {
    changeFenceFunction(90, "natura90180", activeFence);
  });
  ...
  boardsMatSingle[0].addEventListener("click", () => {
    fencesArr[activeFence].color = "braun";
    ...
    if (fencesArr[activeFence].type == "natura180180") {
      for (let i = 0; i < fencesArr.length; i++) {
        if (activeFence != i && fencesArr[i].status != "disposedFence") {
          changeFenceFunction(180, "natura180180", i);
        }
      }
    }
    if (fencesArr[activeFence].type == "natura90180") {
      for (let i = 0; i < fencesArr.length; i++) {
        if (activeFence != i && fencesArr[i].status != "disposedFence") {
          changeFenceFunction(90, "natura90180", i);
        }
      }
    }
